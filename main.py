...
async def on_message_edit(before, after: Message):
    try:
        ...
        logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). Starting search for TB seals.")

        seals = [x.name for x in after.embeds[0].fields]
        logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). Found {len(seals)} seals.")
        ...
        if len(seals) > 0:
            ...
            if len(grouped_members.found_members) > 0:
                logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). Sending notifications to recognized "
                            f"members.")
                await Notifier.notify_members(ctx, grouped_members.found_members)

            if len(grouped_members.unrecognized) > 0:
                logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). Sending notifications to unrecognized "
                            f"members.")
...
async def notag(ctx: Context, value: bool):
    logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). Received 'notag' command with value {value}")
    try:
        ...
        if updated_setting is None:
            ...
            logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). 'notag' value has been changed to "
                        f"{updated_setting}")
            await ctx.send(
...
async def track_c3po_tb(ctx: Context, value: bool):
    logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). Received 'track_c3po_tb' command with value {value}")
    try:
        channel_id = ctx.channel.id
        ...
        if updated_setting is None:
            ...
            logger.info(f"Channel: {ctx.channel.id} ({ctx.channel.name}). 'track_c3po_tb' value has been changed to "
                        f"{updated_setting}")
            await ctx.send(
...
async def setting_update_error(ctx: Context):
    logger.warning(f"Channel: {ctx.channel.id} ({ctx.channel.name}). Unable to update setting value: None")
    await ctx.send(
